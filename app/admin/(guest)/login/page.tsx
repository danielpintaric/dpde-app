...
function AdminLoginForm() {
  ...
  return (
    <div className="mx-auto max-w-sm px-6 py-20">
      <h1 className="font-serif text-2xl tracking-tight text-zinc-100">Admin</h1>
      <p className="mt-2 text-sm text-zinc-500">
        Sign in with Supabase Auth (email + password). Only addresses in{" "}
        <code className="text-zinc-600">ADMIN_EMAILS</code> may access admin.
      </p>
      <form onSubmit={onSubmit} className="mt-8 space-y-4">
        {gateReason === "forbidden" ? (
          <p className="rounded border border-amber-900/40 bg-amber-950/25 px-3 py-2 text-sm text-amber-100/90">
            This account is not authorized for admin. Check <code className="text-amber-200/80">ADMIN_EMAILS</code>{" "}
            on the server.
          </p>
        ) : null}
        {error ? (
...
export default function AdminLoginPage() {
  return (
    <Suspense
      fallback={
        <div className="mx-auto max-w-sm px-6 py-20 text-sm text-zinc-500">Loading…</div>
      }
    >
      <AdminLoginForm />
    </Suspense>
